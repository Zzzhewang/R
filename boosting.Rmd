...
full= trainfeatures %>% 
  inner_join(trainlabels, by='respondent_id')
...
gbm_seasonal <-gbm(seasonal, data = train, distribution = "gaussian")
spredict<-predict(gbm_seasonal,newdata=test,type="response")
mse=mean((test$seasonal_vaccine-spredict)^2)
mse
```
```{r}
h1n1 = as.formula(paste("h1n1_vaccine", paste(features, collapse=" + "), sep=" ~ "))
gbm_h1n1 <- gbm(h1n1, data = train, distribution = "gaussian")
hpredict<-predict(gbm_h1n1,newdata=test,type="response")
...
h1n1_prediction <- predict(gbm_h1n1, Xtest, type="response")
seasonal_prediction <- predict(gbm_seasonal, Xtest, type="response")
submission = data.frame(respondent_id = Xtest$respondent_id,
                           h1n1_vaccine = h1n1_prediction,
                           seasonal_vaccine = seasonal_prediction)
write.csv(submission,"gbmsubmit.csv", row.names = FALSE, quote=FALSE)
```